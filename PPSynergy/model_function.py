...
def train(model, device, dataloader, optimizer, loss_fn):
    ...
    for x_train, y_train in dataloader:
        ...
        optimizer.zero_grad()
        output,diffusion_loss = model(x_train)
        loss = loss_fn(output, y_train)
        loss=loss+0.2*diffusion_loss
        loss.backward()
        optimizer.step()
...
def save_AUCs(AUCs, filename):
    # Write a tab-separated line, rounding floats to 4 decimal places.
    row = [f'{v:.4f}' if isinstance(v, float) else str(v) for v in AUCs]
